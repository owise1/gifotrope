...
module.exports = function(imagePath, cb){
  ...
  fs.mkdir(rand, function(err, ok){
    exec('cp ' + imagePath + ' ' + __dirname + '/' + rand + '; cd ' + __dirname + '/' + rand + '; ' + __dirname + '/gif2anim -c ' + path.basename(imagePath), function(error, stdout, stderr) {
      if (error !== null) {
      ...
      fs.readdir(rand, function(err, allFiles){
        ...
        strip.write(theFile, function(err){
          allFiles.map(addRand).map(function (f) { return fs.unlink(f) });
          fs.rmdir('./' + rand);
          if(cb) cb(theFile);
        });
